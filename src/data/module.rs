...
impl _TripleDataModule {
    ...
    fn new(module: &PyAny, py: Python, config: &PyAny) -> PyResult<Self> {
        ...
        let head_map_py: HashMap<_, _> = head_map
            ...
            .flat_map(|(h, m)| {
                m.iter()
                    .map(move |(r, t)| -> PyResult<((i64, i64), &PyAny)> {
                        Ok((
                            (h as i64, *r as i64),
                            torch
                                .getattr("from_numpy")?
                                .call1((
                                    Array1::from_iter(t.iter().map(|i| *i as i64)).to_pyarray(py),
                                ))?
                                .getattr("cuda")?
                                .call0()?,
                        ))
        ...
        let tail_map_py: HashMap<_, _> = tail_map
            ...
            .flat_map(|(t, m)| {
                m.iter()
                    .map(move |(r, h)| -> PyResult<((i64, i64), &PyAny)> {
                        Ok((
                            (t as i64, *r as i64),
                            torch
                                .getattr("from_numpy")?
                                .call1((
                                    Array1::from_iter(h.iter().map(|i| *i as i64)).to_pyarray(py),
                                ))?
                                .getattr("cuda")?
                                .call0()?,
                        ))
