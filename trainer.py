...
class Trainer():
    ...
    def validation(self, epoch):
        ...
        for iter, batch_data in enumerate(self.val_data_loader):
            # fetch batch data
            try:
                src_id, pos_id, mask_label, mask_pos = batch_data
            except RuntimeError:
                print("One data instance's length should be 4, received {}.".format(len(batch_data)))
                continue
            if self.use_cuda:
                src_id, pos_id, mask_label, mask_pos = \
                    src_id.to(self.device), pos_id.to(self.device), mask_label.to(self.device), mask_pos.to(self.device)
            input_x = {
                'src_ids': src_id,
                'position_ids': pos_id,
                'mask_pos': mask_pos
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, batch_data in enumerate(self.train_data_loader):
                ...
                acc = logits.max(dim=1)[1].eq(label.squeeze()).sum()
