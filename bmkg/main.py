...
train_g.add_arg("warmup_proportion", float, 0.05,
...
def main():
    ...
    if args.bmtrain:
        ...
        val_loader = DistributedDataLoader(dataset=val_dataset, batch_size=args.batch_size, shuffle=True)
    else:
    ...
    if args.bmtrain:
        ...
        scheduler = bmt.lr_scheduler.Noam(
            optimizer,
            start_lr=args.warmup_proportion * args.learning_rate,
            warmup_iter=args.warm_up_epochs,
            end_iter=args.epoch)
        ...
        if args.lr_scheduler == "warmup_cosine":
            ...
            scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.1, total_iters=args.warm_up_epochs)
        else:
